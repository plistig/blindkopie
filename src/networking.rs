...
impl Default for SafeHttpsClient {
    fn default() -> Self {
        let resolver = AsyncHyperResolver::new(
...
impl SafeHttpsClient {
    ...
    pub async fn websocket(
        &self,
        ws_uri: Uri,
    ) -> Result<(WebSocketStream<MaybeHttpsStream<TcpStream>>, Response<()>)> {
        let mut http_uri = ws_uri.clone().into_parts();

        let mut ws_uri = ws_uri.into_parts();
        match ws_uri.scheme {
            Some(s)
                if s.as_str().eq_ignore_ascii_case("http")
                    || s.as_str().eq_ignore_ascii_case("ws") =>
            {
                ws_uri.scheme = Some("ws".parse()?);
            }
            Some(s)
                if s.as_str().eq_ignore_ascii_case("https")
                    || s.as_str().eq_ignore_ascii_case("wss") =>
            {
...
pub async fn with_timeout_and_cancellation_token<T, E, F>(
    state: &State,
    timeout_seconds: u64,
    future: F,
) -> Result<T>
    ...
    fn branch(self) -> ControlFlow<Self::Residual, Self::Output> {
        match self {
            AwaitWithCancellationTokenOutcome::Cancelled => {
                ControlFlow::Break(AwaitWithCancellationTokenCancelled)
            }
...
pub async fn await_with_cancellation_token(
    state: &State,
    notify: &Notify,
) -> AwaitWithCancellationTokenOutcome {
    select! {
...
pub async fn sleep_with_cancellation_token(
    state: &State,
    secs: u64,
) -> AwaitWithCancellationTokenOutcome {
    select! {
...
pub fn parse_url(url: &str) -> Result<Uri> {
    ...
    Ok(Uri::from_str(
        &utf8_percent_encode(url, &NEEDS_ESCAPING).to_string(),
    )?)
}
